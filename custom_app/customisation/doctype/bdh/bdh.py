...
#		self.validate_child_table()
		...
		# Check if the child table is completed
#		child_table_completed = all(item.parameter and item.done and item.employee and item.date for item in self.check_items)

		# Allow editing of the "approval" field if the child table is completed
#		if child_table_completed:
#			frappe.db.set_value(self.doctype, self.name, "approval", "", update_modified=False)
#		else:
#			frappe.db.set_value(self.doctype, self.name, "approval", "Approved", update_modified=False)
...
#		frappe.msgprint("Get Template")
		# Fetch the template name from the BDH document
		# Clear existing check_items
